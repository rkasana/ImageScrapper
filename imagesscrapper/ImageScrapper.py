...
class ImageScrapper:
    ...
    def createURL(keyword):
        ...
        url = "https://www.bing.com/images/search?q=" +keyword+"&qpvt=images&FORM=IGRE"
        return url
    ...
    def getimageUrlList(rawHtml):
        imageUrlList = []
        for a in rawHtml.find_all("a", {"class": "iusc"}):
            mad = json.loads(a["m"])
            link = mad["turl"]

            m=json.loads(a["m"])
            imageExtension = m["murl"]
            # link, imageExtension = json.loads(a["m"]["turl"], json.loads(a.text)["ity"]
